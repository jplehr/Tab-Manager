	...
	search:function(e){
		...
		if(!searchLen){
			...
			for(var id in idList){
				...
				var exprs = (tab.title+tab.url).toLowerCase().split(" ");
				var searchStrs = e.target.value.toLowerCase().split(" ");
				hitMap[id] = 0; // Map to mark search hits
				for(var exprId in exprs) {
					for(var searchID in searchStrs){
						if(exprs[exprId].indexOf(searchStrs[searchID]) >= 0){
							hitMap[id] += 1;
						}else{
							hitMap[id] += 0;
						}
				...
				if((hitMap[id] / searchStrs.length) >= 0.8) {
